...
class QaSlideServiceTest {
    ...
    void create_success() {
        ...
        var notified = slide(null, saved.barcode(), request.activationCode());
        ...
        when(notificationService.notifyEntityChange(eq("qaSlide"), isNull(), eq(notified))).thenReturn(Mono.empty());
        ...
        verify(notificationService).notifyEntityChange(eq("qaSlide"), isNull(), eq(notified));
    }
