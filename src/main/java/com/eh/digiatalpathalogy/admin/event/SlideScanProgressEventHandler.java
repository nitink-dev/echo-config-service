 ...
 */
public class SlideScanProgressEventHandler implements TopicHandler {
    ...
    public SlideScanProgressEventHandler(String topic, ObjectMapper objectMapper, SlideScanStatusService slideScanStatusService) {
        this.topic = topic;
    ...
    public Mono<Void> handle(KafkaEnvelope env) {
        ...
        return Mono.fromCallable(() -> objectMapper.readValue(env.payload(), SlideScanProgressEvent.class))
                .doOnNext(event -> log.debug("Slide scan progress event received | topic={} partition={} offset={} key={}", env.topic(), env.partition(), env.offset(), env.key()))
                .flatMap(slideScanStatusService::streamSlideScanStatus)
                .doOnSuccess(v -> log.debug("Slide scan progress processed successfully | offset={} key={}", env.offset(), env.key()))
                .onErrorMap(ex -> {
